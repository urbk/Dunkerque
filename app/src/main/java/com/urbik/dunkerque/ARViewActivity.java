...
//	}
    ...
    public void createGeometryModel(EnumModel EnumModel[]) {
        try {
            for (EnumModel m : EnumModel) {
                Model tempM = m.getModel();
                if (tempM.getDisplayType() == Model.VISUAL_HELP) {
                    tempM.setIGeometry(metaioSDK.createGeometry(tempM.getContentToDisplay()));
                    tempM.getIGeometry().setTransparency(0.8f);
                } else {
                    tempM.setIGeometry(metaioSDK.createGeometry(tempM.getContentType().getLocation() + tempM.getContentToDisplay()));
                }
                if (tempM.getDisplayType() == Model.OCCLUSION) {
                    tempM.getIGeometry().setOcclusionMode(true);
                }
                if (tempM.getContentType() == ContentType.Location) {
                    tempM.getIGeometry().setTranslationLLA(tempM.getmLla());
                }
                if (tempM.getIGeometry() != null) {
                    tempM.getIGeometry().setScale(tempM.getScale());
                } else
                    MetaioDebug.log(Log.ERROR, "Error loading geometry: " + tempM.getContentType().getLocation() + " " + tempM.getContentToDisplay());
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    void onScreenshot(String pathStorage, String folderName) {
        ...
        try {
            ...
            Toast.makeText(getApplicationContext(), com.urbik.dunkerque.R.string.ScreenshotSuccess, Toast.LENGTH_SHORT).show();
//            Utils.shareScreenshot(this, imagePath);
        } catch (Exception e) {
            Toast.makeText(getApplicationContext(), com.urbik.dunkerque.R.string.ScreenshotFail, Toast.LENGTH_SHORT).show();
        }
    ...
    public boolean onSwitchDisplayedModel(Map<String, EnumModel> m, String id, int numberOfModel, boolean right) {
        ...
        if (subS[2].equals("D")) {
            for (Map.Entry<String, EnumModel> entry : m.entrySet()) {
                ...
                String keyValue = value[0] + "-" + value[1];
                if (keyValue.equals(subS[0] + "-" + subS[1]) && value[2].equals("D") && entry.getValue().getModel().getIGeometry().isVisible() && !found) {
                    found = true;
                    int currentNumber = Integer.valueOf(value[3]);
